    ...
    return (
        ...
        >
            <img
                ...
                alt={node.attrs.alt || ''}
                draggable={false}
                className={`max-w-full rounded-lg transition-all select-none ${selected ? 'ring-2 ring-purple-500' : ''}`}
            />
            {(showDelete || selected) && (
                <button
                    type="button"
            ...
            {showToolbar && (
                <div className="flex items-center px-3 py-2 border-b border-white/10 bg-black/20 gap-1 flex-wrap">
                    <button
                        ...
                        className={`p-2 rounded transition-colors ${editor.isActive('bold') ? 'bg-purple-500/30 text-purple-400' : 'text-gray-400 hover:text-white hover:bg-white/10'}`}
                        title="Negrita"
                    >
                    ...
                    <button
                        ...
                        className={`p-2 rounded transition-colors ${editor.isActive('italic') ? 'bg-purple-500/30 text-purple-400' : 'text-gray-400 hover:text-white hover:bg-white/10'}`}
                        title="Cursiva"
                    >
                    ...
                    <button
                        ...
                        className={`p-2 rounded transition-colors ${editor.isActive('heading', { level: 2 }) ? 'bg-purple-500/30 text-purple-400' : 'text-gray-400 hover:text-white hover:bg-white/10'}`}
                        title="Título"
                    >
                    ...
                    <button
                        ...
                        className={`p-2 rounded transition-colors ${editor.isActive('bulletList') ? 'bg-purple-500/30 text-purple-400' : 'text-gray-400 hover:text-white hover:bg-white/10'}`}
                        title="Lista"
                    >
                    ...
                    <button
                        ...
                        className={`p-2 rounded transition-colors ${editor.isActive('orderedList') ? 'bg-purple-500/30 text-purple-400' : 'text-gray-400 hover:text-white hover:bg-white/10'}`}
                    ...
                    <button
                        type="button"
                        onClick={addLink}
                        className={`p-2 rounded transition-colors ${editor.isActive('link') ? 'bg-purple-500/30 text-purple-400' : 'text-gray-400 hover:text-white hover:bg-white/10'}`}
                        title="Enlace"
                    >
                    ...
                    <button
                        ...
                        className={`p-2 rounded transition-colors ${editor.isActive('blockquote') ? 'bg-purple-500/30 text-purple-400' : 'text-gray-400 hover:text-white hover:bg-white/10'}`}
                        title="Cita"
                    >
            ...
            <div className="flex items-center justify-between px-4 py-2 border-t border-white/10 bg-black/20">
                ...
                {onSubmit && (
                    <button
                        ...
                        className="flex items-center gap-2 px-4 py-2 bg-purple-600 hover:bg-purple-700 disabled:bg-gray-700 disabled:text-gray-500 text-white font-medium rounded-lg transition-colors"
                    >
                        {isSubmitting ? (
                            <>
                                <Loader2 className="w-4 h-4 animate-spin" />
...
const MarkdownEditor: React.FC<Omit<ForumEditorProps, 'category'>> = ({
    ...
    const renderPreview = (text: string) => {
        ...
        return text
            ...
            .replace(/`([^`]+)`/g, '<code class="bg-black/40 px-1 rounded text-purple-400 text-sm">$1</code>')
            .replace(/\*\*([^*]+)\*\*/g, '<strong class="font-bold text-white">$1</strong>')
            .replace(/\*([^*]+)\*/g, '<em class="italic">$1</em>')
            .replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<a href="$2" class="text-purple-400 underline">$1</a>')
            .replace(/!\[([^\]]*)\]\(([^)]+)\)/g, '<img src="$2" alt="$1" class="max-w-full rounded my-1 max-h-48" />')
            .replace(/^&gt; (.+)$/gm, '<span class="border-l-2 border-purple-500 pl-2 text-gray-400 italic block">$1</span>')
