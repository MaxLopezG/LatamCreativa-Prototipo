 ...
 * Incluye icono, nombre, descripción y conteo de hilos.
 */
...
export const ForumCategoryCard: React.FC<ForumCategoryCardProps> = ({ category, onClick }) => {
    ...
    return (
        <div
            onClick={handleClick}
            className={`group relative bg-[#1a1a2e]/60 hover:bg-[#1a1a2e]/80 border border-white/5 hover:border-white/10 rounded-xl p-5 cursor-pointer transition-all duration-300 hover:scale-[1.02] hover:shadow-xl`}
        >
            {/* Icon + Title */}
            <div className="flex items-start gap-4">
                <div className={`p-3 rounded-xl ${colorClasses.bg} ${colorClasses.border} border transition-all group-hover:scale-110`}>
            ...
            <div className="mt-4 pt-3 border-t border-white/5">
                <div className="flex items-center justify-between">
                    ...
                    {category.lastActivity && (
                        <span className="text-gray-500 text-xs truncate max-w-[60%]" title={category.lastActivity.threadTitle}>
