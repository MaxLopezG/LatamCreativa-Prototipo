...
import { Menu, UploadCloud, Bell, Plus, FileText, Layers, Video, Box, Users, Search, Command, Briefcase, MessageCircleQuestion, CalendarDays, Heart, UserPlus, Check, ShoppingCart, Building2, X } from 'lucide-react';
...
export const Header = ({ 
  ...
  const [isMobileSearchOpen, setIsMobileSearchOpen] = useState(false); // Estado para búsqueda móvil
  ...
  const handleSearchSubmit = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter' && searchQuery.trim()) {
      ...
      setIsMobileSearchOpen(false); // Cerrar al buscar
    }
  };
  ...
  const createOptions = [
    { path: '/create/article', icon: FileText, label: 'Escribir Artículo', desc: 'Blog', color: 'text-blue-500', bg: 'bg-blue-500/10' },
    { path: '/create/portfolio', icon: Layers, label: 'Subir Proyecto', desc: 'Portafolio', color: 'text-amber-500', bg: 'bg-amber-500/10' },
  ...
  return (
    <header className="flex h-20 w-full items-center justify-between border-b border-slate-200 dark:border-white/[0.06] bg-white/80 dark:bg-[#030304]/80 px-6 md:px-8 backdrop-blur-xl absolute top-0 right-0 left-0 z-40 transition-colors">
      ...
      {/* Mobile Search Overlay (Replaces Logo/Menu when active) */}
      {isMobileSearchOpen ? (
         <div className="flex w-full items-center gap-4 animate-fade-in">
             <div className={`relative flex-1`}>
                ...
                <input 
                    ...
                    placeholder="Buscar..." 
                    className={`block w-full pl-10 pr-4 py-2.5 border rounded-xl text-sm font-medium bg-white dark:bg-[#0A0A0C] ${accentBorder} text-slate-900 dark:text-white focus:outline-none`}
                />
             </div>
          ...
          <>
            ...
            <div className="flex items-center gap-4 w-auto md:w-1/3">
                ...
                <button onClick={() => setIsMobileSearchOpen(true)} className="md:hidden text-slate-500 dark:text-slate-400 p-1">
            ...
            {/* RIGHT: Actions */}
            <div className="flex items-center justify-end gap-3 md:gap-4 w-auto md:w-1/3">
                {/* Shopping Cart */}
                <button 
                    onClick={onCartClick}
