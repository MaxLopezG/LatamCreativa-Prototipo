...
export const VideoContent: React.FC<VideoContentProps> = ({ state, actions }) => {
  ...
  const { activeModule, activeCategory, createMode, viewingAuthor, searchQuery, cartItems, contentMode } = state;
  const {
    setActiveCategory,
    handleModuleSelect: onModuleSelect,
    setViewingAuthor: onAuthorClick,
    setCreateMode,
  ...
  const handleAuthorClick = (author: string | { name: string, avatar?: string }) => {
    if (typeof author === 'string') {
      onAuthorClick?.({ name: author });
    } else {
  ...
  const renderContent = () => {
    ...
    if (viewingAuthor) {
      return (
        <UserProfileView
          key={viewingAuthor.id || 'profile-view'} // Force remount on author change
